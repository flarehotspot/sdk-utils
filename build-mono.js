...
const outfile = process.platform == 'win32' ? 'main.exe' : 'main.app';
...
(async function () {
  ...
  await execPromise(
    `node make-go.work.js && cd main && go build --tags="${buildTags}" ${ldflags} -trimpath -o ${outfile} main_mono.go`
  );

  console.log(`Built app successfully: main/${outfile}`);
})();
